...
var testTaskManager TaskManager
...
func TestInitializeTaskManager(t *testing.T) {
	...
	ctx = context.WithValue(ctx, ContextKey("testContextProperties"), testContextProperties)
	testTaskManager = New(ctx, TaskManagerTestDataUrl, map[string]TaskWorkflowDefinition{
		"TaskType": DefaultTaskWorkflow,
	})
}
